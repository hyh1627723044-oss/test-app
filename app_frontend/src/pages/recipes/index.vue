  ...
  <view class="page-shell">
    ...
    <view class="search-card paper-card">
      <view class="search-row">
        <input v-model="keyword" class="search-input" placeholder="搜索菜名 / 标签" />
        <image class="search-cat" src="/static/illustrations/search-cat.png" mode="aspectFit" />
      </view>
      <scroll-view scroll-x class="tag-scroll"><view class="tag-row"><text v-for="tag in quickTags" :key="tag" class="quick-tag" :class="{ selected: activeTag === tag }" @tap="activeTag = activeTag === tag ? '' : tag">{{ tag }}</text><text class="quick-tag plus">+</text></view></scroll-view>
    </view>

    <view class="section-row"><scroll-view scroll-x class="category-scroll"><view class="category-row"><text v-for="category in categories" :key="category" class="category" :class="{ selected: activeCategory === category }" @tap="activeCategory = category">{{ category }}</text></view></scroll-view><view class="add-button"><image src="/static/illustrations/icon-recipe-add.png" mode="aspectFit" /><text>添加菜谱</text></view></view>
    ...
    <view v-for="recipe in filteredRecipes" :key="recipe.id" class="recipe-card paper-card">
      ...
      <view class="recipe-copy"><text class="recipe-title">{{ recipe.title }}</text><text class="time">◷ {{ recipe.time }} 分钟</text><text class="description">{{ recipe.description }}</text><view class="tags"><text v-for="tag in recipe.tags" :key="tag">#{{ tag }}</text></view><view class="card-footer"><view /><button class="plan-button" size="mini">加入计划</button></view></view>
    </view>
...
const recipes = ref([
  { id: 1, title: '番茄炒蛋', time: 12, description: '酸甜开胃，简单快手，一碗暖心的家常菜。', tags: ['家常', '快手'], tape: 'yellow', decor: 'flower', cover: '' },
  { id: 2, title: '香煎豆腐', time: 15, description: '外焦里嫩，简单调味，豆香浓郁又下饭。', tags: ['清淡', '素食'], tape: 'green', decor: 'leaf', cover: '' },
  { id: 3, title: '虾仁蒸蛋', time: 18, description: '嫩滑营养，做法简单，大小朋友都喜欢。', tags: ['高蛋白', '蒸菜'], tape: 'pink', decor: 'star', cover: '' },
])
